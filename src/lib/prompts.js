...
export const getScenarioPrompt = ({
  ...
  const effectiveLocationPlan = locationPlan || {
    ...
    guidance: customLocation.trim()
      ...
      : '題材本文と4コマの行動に最も適した安全で具体的な場所を考案すること。'
  };
  ...
  const contextRebootLock = inputMode === 'manual'
    ? `(Topic Lock): Focus strictly on the user-provided topic below.
         (Source Boundary Lock): Interface labels, input-method metadata, and topics from previous sessions are not story content. Do not place them in Topic, dialogue, actions, props, signs, or VisualEvidence unless the user explicitly included them in the topic.`
  ...
  const gagTopicSelectionRules = inputMode === 'manual'
    ? `1. ユーザー提供トピックを、入力方法や分野を理由に別の題材へ置き換えないこと。
         2. 入力本文にある固有名詞、数値、役割、因果を保持し、本文にないニュース、発表、出来事を創作しないこと。
         3. 笑いは題材そのものの置換ではなく、人物の行動、受け止め方、結果のずれから作ること。
         - ギャグの大人しい現実解への矯正は禁止。題材の核と明示条件を守る限り、物・設備・舞台の大破壊、因果の飛躍、支離滅裂、不条理、理不尽、シュールな静寂を強く許可する。辻褄合わせや後付け説明で狂気と勢いを弱めないこと。`
  ...
  return `
         ...
         3. 抽象的な悲しさだけで終わらせず、題材に固有の場所、小道具、行動、結果を画面に置くこと。` : gagTopicSelectionRules}
         4. **【場所（Location）の選定義務】**:
        ...
        ${adaptiveLocationMode ? `4. **【内容適合舞台設計 (Story-Fit Location)】**:
           ...
           - 題材や出来事の中心が実際に起きる場所、または登場人物がその出来事に自然に関われる場所を選べ。単に絵映えするだけの無関係な場所は禁止。
        ...
        4.8 **【出来事を証明する視覚証拠 (Visual Story Evidence)】**:
           - 元の題材本文から、その出来事であることを絵だけで証明できる、短く具体的で描画可能な名詞を3〜5個抽出せよ。
...
${styleJson.anti_patterns ? `            - 絶対禁止事項:\n${styleJson.anti_patterns}` : ''}
          ...
          3.5 **【笑いの構造設計 (Comedy Structure Engineering)】**:
             ...
             - ${punchlineType === 'Auto' ? `**【オチの多様化 (Punchline Variety Enforcement)】**: 4コマ目のオチが毎回同じパターンにならないよう、以下の9系統からネタに最適なものを選択せよ。
                ...
                **【禁止事項】**: 舞台を元ネタと無関係な場所（宇宙、異世界、RPGなど）に移すこと。元ネタに登場しない架空の出来事を捏造すること。題材の内容を「たとえ話」にすり替えること。` : ''}
          ...
          [SNS_EXPLANATION]
          [SNS投稿用に、今回選んだ題材の短い見出しと150〜250字程度の解説を日本語で記載。検索で確認できた内容、またはユーザー提供文に実際に書かれた内容だけを要約する。漫画内の架空の出来事をニュースの事実として書かない。未確認・予定・推測はその性質を保持し、ユーザー提供文の創作は創作の紹介として書く。深刻な題材は茶化さない。出典URLは別途付けるので、この欄にURLや出典記号を書かない。前置きやハッシュタグは不要。]
          [創作である旨の注意書きを付ける場合は、本文の後で改行し、次の行に独立した丸括弧「（…）」で記す。事実を説明する本文と区別する。]
          [/SNS_EXPLANATION]
          Topic: [${inputMode === 'manual' ? 'ユーザー提供トピックの内容を表す自然な見出し（15文字以内）。入力方法やUIラベルを題名にしない' : 'ニュースの見出し（15文字以内）'}]
          ...
          - (許可事項): 題材の当事者（スポーツ選手、政治家、怪人、動物、虫など）や名もなきモブキャラは、物語を面白くするために必要であれば自由に登場・発言させて構わない。
          - (ハルシネーション防止): ゲストを登場させる場合でも、画像生成時のVisual Actionに「意味不明な文字」や「描画不可能な複雑すぎる行動」が混入しないよう、シンプルで視覚的にわかりやすい行動に留めること。
          - 構成: 起承転結（4段）。
           - 内容: ${isSeriousDocumentary ? '原文の事実を変えない真摯な再構成。4コマ目だけ、事実に対するシリアスな反応または結論で締める。' : isGeneralSerious ? '人物の感情と出来事の因果を保つシリアスな構成。4コマ目は選択した余韻、決意、警告、問題提起、救い、または喪失で締める。' : '題材に対する辛辣な風刺や、キャラの個性を活かしたドタバタ劇。'}
         ...
         1. [前提事実と現在の問題点列挙]
             - ${inputMode === 'manual' ? 'ユーザー提供トピック' : 'ニュースのトピック'}、および指定された場所や服装などの制約条件を列挙せよ。
             - 思考トレースでは「ユーザー提供トピック」と明記する。ユーザー提供文を未検証の外部ニュースや確定事実として書かない。
          2. [実行パスのトレース]
