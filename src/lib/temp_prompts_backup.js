...
【Guard S: 五感バランスの強制（ト書き品質革命）】
ト書き（状況・アクション描写）が「視覚情報だけ」に偏ることを禁止する。各コマの状況欄に、以下の非視覚情報を最低1つ含めよ:
- 聴覚: 足音、ドアが軋む音、遠くの喧騒、衣擦れ、心拍音
- 触覚: 風の冷たさ、汗の滴り、紙の手触り、握りしめた拳の痛み
- 嗅覚: 料理の匂い、雨上がりの湿った空気、古い本の匂い
- 体内感覚: 胃がキュッとする、背筋が凍る、血が頭に昇る
4コマ全体で聴覚・触覚・嗅覚・体内感覚のうち最低2種類を使用せよ。五感描写は1コマ1文以内で簡潔に。

【Guard M: 比喩素材の世界観準拠】
使い古された定型比喩（「時間が止まった」「心臓が口から出そう」「鉛のように重い」「雷に打たれた」「氷のような視線」「火花が散る」「闇に飲まれる」）を完全禁止。代わりにそのコマの舞台(Location)に由来する素材から比喩を構築せよ（例: ラーメン屋→「麺のように伸びきった沈黙」）。1シナリオ中の比喩は最大2つまで。
...
${styleJson.anti_patterns ? `            - 絶対禁止事項:\n${styleJson.anti_patterns}` : ''}
          ...
          【シナリオ構成・演出の絶対厳守 (v2.99 Alpha)】
           ...
           1. **「Show, Don't Tell」のギャグ特化 & セリフ品質ルール**:
                - **セリフの質**: 状況説明セリフ禁止。セリフは「感情・ツッコミ・ボケ」に特化せよ。建前と本音のズレ（サブテキスト）を活用し、絵とセリフの矛盾で笑いを作れ。AI的な定型表現（「やれやれ」「ふふっ」「まあいいか」「〜な一日だった」）は完全禁止。
                - **ト書きの質**: 感情を物理アクションに変換（「驚く」→「魂が抜ける」、「怒る」→「顔から湯気」等）。ト書き内に「（リアクション: キャラ名→表情）」形式で群像リアクションを最低2コマに含めよ。
                - **漫符ルール**: 汗マーク等は視覚シンボルのみ。英語ラベル("POPPING VEIN"等)の描画禁止。
                - **オチの多様化**: 白目オチ(BLANK)の乱用を避け、ツッコミ・爆笑・冷静な呆れ(NORMAL)等の多様なリアクションでオチを作れ。
  
            2. **テキストの量的制限 (Dialogue Density Control)**:
               - **【最重要・セリフ密度の下限】**: 4コマ全体で**最低8個のフキダシ（セリフ）**を確保せよ。セリフのないコマは禁止。全コマに**最低2つのセリフ**を書け。掛け合い・ツッコミ・ボケの応酬でシナリオを賑やかにすること。
               - **上限**: 1コマあたりのフキダシは**最大3つまで**。
               - **セリフの長さ**: 短く鋭い一文に収めよ。ただし「短い」は「少ない」ではない。**数は多く、一つ一つが短い**のが理想。
               - **禁止**: セリフ内にト書き（「(怒って)」等）を入れるな。
                - **句読点ルール**: セリフ末尾は必ず終止記号(。/！/？/♪/♡/っ/！？)で締めよ。修飾記号(……/〜/ー)の後には終止記号を添えよ。同じ文末表現が3セリフ以上連続禁止。「……」系は1シナリオ中2回まで。
          ...
          3.5 **【笑いの構造設計 (Comedy Structure Engineering)】**:
              - **大原則**: 笑い＝「予想」と「実際」のズレ。以下の技法を最低2つ組み合わせよ。
              - **【GMC+S ギャグ設計】**: キャラの欲求(Goal)→阻む理不尽な障害(Conflict)→達成できない場合の切実なリスク(Stakes)→欲求が最悪に裏切られるオチ(Resolution)。
              - **【ズレ技法（最低2つ選択）】**: 置換(文脈の入れ替え) / 誇張(極端化) / 逆転(立場の入れ替え) / 不条理(脈絡のない狂った要素) / 緊張と緩和(張り詰め→崩壊) / 常識に戻る(1人だけ冷静)
              - **【4コマへの構成マッピング】**:
                * 1コマ目＝フリ（読者の予想を作る。ここでは笑わせない）
                * 2コマ目＝ボケ開始（ズレた行動・発言で逸脱）
                * 3コマ目＝溜め/かぶせ（ボケ加速。天丼の変奏も有効）
                * 4コマ目＝オチ（全笑いエネルギーをここに集中投下）
              - **【推奨テクニック】**: 天丼(1コマ→3コマ変奏→4コマ限界突破) / ノリツッコミ(乗ってから崩す) / かぶせ(畳み掛け) / 自虐(弱点ネタ)
              - **【ギャグ強度】**: 強烈なボケ最優先。おとなしいギャグ禁止。カオスでも勢いで笑わせろ。ただし意味不明は避けよ。
               - **【ビジュアルギャグ連動】**: ズレ技法に応じた視覚演出を「状況」欄に仕込め。誇張→身体変形・爆発リアクション、逆転→普段と真逆のEMOTION、緊張→絵柄急変、不条理→背景に脈絡のない巨大物体。テキストだけでなく絵でも笑いを表現するのが漫画の鉄則。
             - ${punchlineType === 'Auto' ? `**【オチの多様化 (Punchline Variety)】**: 以下の10系統からネタに最適なオチを選べ: 爆発型(全員暴走→IMPACT) / 静寂型(無言で固まる→NORMAL,CHIBI_GAG) / 感動詐欺(狂気のイイ話→SHOUJO) / メタ崩壊(次元の壁破壊→IMPACT) / 理不尽制裁(まともなキャラが被害→GEKIGA) / 天丼爆発(繰り返し限界突破→IMPACT) / 夢オチ(現実との落差→SHADOW) / サイコホラー(狂気発覚→HORROR) / 盛大な勘違い(前提崩壊→CHIBI_GAG) / 打ち切りエンド(強制終了→IMPACT)。⚠️BLANK(白目)は極限の絶望のみ、安易に乱用するな。` :
          ...
          5. **【感情絵柄タグ (Emotion Style Tag)】**:
              - 各コマの冒頭に[EMOTION: XXX]タグを**必ず1つ**付与。4コマで最低**2種類以上**を使い分けよ。
              - 選択肢: NORMAL(日常・真顔) / CHIBI_GAG(ツッコミ・ギャグ) / GEKIGA(劇画・シリアス) / SHOUJO(キラキラ感動) / HORROR(恐怖) / BLANK(白目・魂抜け※極限の絶望のみ、乱用禁止) / IMPACT(爆発・衝撃) / WATERCOLOR(回想) / RETRO(レトロ) / GLITTER(ドヤ顔) / SHADOW(不穏) / THICK_PAINT(重厚) / PASTEL(ほのぼの) / CEL(セル画) / DARK_ANIME(ダーク) / THIN_LINE(繊細) / HIGH_SATURATION(ビビッド)
              - 4コマ目のオチはNORMAL以外を優先。BLANKの代わりにNORMAL(真顔・点目)やCHIBI_GAG(ちび呆れ)も有効。

          6. **【カメラ演出タグ (Camera Direction Tag)】**:
              - 各コマに[Camera: XXX]タグを付与。タグ名の直後に**物理描写を1文**必ず書け（カメラ名だけは禁止）。
              - 物理描写: 「カメラがどこにあり」「何が巨大/歪み/光の当たり方」を具体的に1〜2文で書け:
