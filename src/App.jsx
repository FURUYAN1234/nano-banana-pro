...
export default function App() {
  const SYSTEM_VERSION = "v1.8.31 Alpha";
  ...
  3.  抽象的な「最近の流行」ではなく、「◯◯が××を発表」といった固有名詞を含むニュースを優先。
        ...
        1. **「原則: 語るな、見せろ」 (Show, Don't Tell... but Explain if Needed)**:
           - 基本は絵での表現を優先するが、**「絵だけで伝わらない重要な文脈」**はモノローグやセリフで補足説明せよ。
           - 「読者が置いてきぼりになる」のを防ぐため、必要な状況説明は許可する。

        2. **テキストと視覚のバランス (Adaptive Text Density)**:
           - **重要**: 視覚効果を阻害しない範囲で、ストーリー理解に必要なセリフを配置せよ。
           - 「全てを説明する」のはNGだが、「訳が分からない」のもNG。適切なバランスを保て。
           - 擬音は引き続き**「カラフルな手書きオノマトペ」「空間に浮かぶ3D文字」**で画面を埋めよ。既存フォントは使うな。
...
- Vertical Force: Strictly VERTICAL Japanese dialogue(Kanji / Kana) only.
  ...
  FINAL_PROMPT = \`
      ...
      (Text Density): **ADAPTIVE**. Visuals first, but **ALLOW EXPLANATORY TEXT** if the situation is unclear.
      ...
      (Dialogue): VERTICAL Japanese text. Use speech bubbles or monologues to **explain the context** so the reader is not confused.
