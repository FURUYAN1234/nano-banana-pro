                              ...
                              <div className="rounded-lg border border-slate-600/60 bg-slate-950/35 p-3">
                                ...
                                <div className="mt-3 rounded border border-amber-400/40 bg-amber-950/25 p-3 text-[10px] leading-relaxed text-slate-200">
                                  ...
                                  <ol className="mt-3 space-y-1.5 list-decimal pl-4">
                                    ...
                                    <li><code>4. API設定＋画像変換＋H3プロンプト生成（同一Provider）</code> ノードで <code>OpenAI API</code> または <code>Google Gemini API</code> を選び、「<strong>🔐 APIキー未登録／登録</strong>」から利用者自身のキーを登録します。ワークフロー読込時、プルダウンで選択中のProviderが未登録なら、そのProviderの入力ダイアログが自動で開きます。キーはワークフローJSON、配布ZIP、設定ファイル、ディスクには保存されず、接続中のComfyUIサーバーのプロセスメモリだけに保持されます。ワークフローのシート移動や別ワークフローへの切替では残り、ComfyUIアプリ／サーバーを終了または再起動すると消去されるため、次回は再入力してください。1回の実行では、選択中の同一Providerが画像変換とH3プロンプト作成の両方に使われ、認証確認または実行時だけ、そのComfyUIサーバーから選択したAPIへ送信されます。</li>
