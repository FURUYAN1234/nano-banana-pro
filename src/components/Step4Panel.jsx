...
const COMFYUI_WORKFLOW_FILENAME = 'Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06-r5.json';
const COMFYUI_WORKFLOW_DOWNLOAD_URL = `${import.meta.env.BASE_URL}workflows/${COMFYUI_WORKFLOW_FILENAME}`;
const COMFYUI_CUSTOM_NODE_FILENAME = 'MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06-r5.zip';
                              ...
                              <div className="rounded-lg border border-slate-600/60 bg-slate-950/35 p-3">
                                ...
                                <div className="mt-3 rounded border border-amber-400/40 bg-amber-950/25 p-3 text-[10px] leading-relaxed text-slate-200">
                                  ...
                                  <ol className="mt-3 space-y-1.5 list-decimal pl-4">
                                    <li>「同梱カスタムノード3点・導入セットをダウンロード」を押して <code>MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06-r5.zip</code> を保存します。</li>
                                    ...
                                    <li>音声2ステップ補正には別途 <code>ComfyUI-H3-AudioRefine</code> が必要です。Managerまたは配布元から <code>ComfyUI/custom_nodes/ComfyUI-H3-AudioRefine/</code> へ導入します。</li>
                                    <li>下のボタンから <code>Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06-r5.json</code> を保存し、<code>ComfyUI/user/default/workflows/</code> 以下へ配置します。</li>
                                ...
                                <div className="mt-3 space-y-1 text-[10px] leading-relaxed text-slate-300">
                                  <p><strong>既定の設定</strong>：4コマを物語の4幕として扱い、台詞1本につき5秒で上限なしの完全可変尺にします。台詞がない場合だけ既定30秒です。映像はFused 4ステップ、音声補正は2ステップ（denoise 0.5）、既定はH3生成BGMありです。r5は区間ごとに生成・検査し、分身、明らかな髪型違い、台詞破綻、二重発声が出た区間だけ原因別に最大3回試行します。MiniMax H3が低音量の物語向けBGMを生成し、台詞中は強く抑えます。提供JSONの設定値は変更せずに配布します。</p>
                                  <p>4つの不足モデルはComfyUIの不足モデル表示からダウンロード候補を開けます。<code>H3 SLA Attention</code> には別途 <code>ComfyUI-PlagueKind-Nodes</code> とTriton対応環境が必要です。音声補正には <code>ComfyUI-H3-AudioRefine</code> も必要です。</p>
