...
const COMFYUI_WORKFLOW_FILENAME = 'FourPanel_NonLM_4step_20260910101338_v5.9.7.json';
const COMFYUI_WORKFLOW_DOWNLOAD_URL = `${import.meta.env.BASE_URL}workflows/${COMFYUI_WORKFLOW_FILENAME}`;
const COMFYUI_CUSTOM_NODE_FILENAME = 'ComfyUI_H3_FourPanel_NonLM_20260910101338_v5.9.7.zip';
                              ...
                              <div className="rounded-lg border border-slate-600/60 bg-slate-950/35 p-3">
                                ...
                                <div className="mt-3 rounded border border-amber-400/40 bg-amber-950/25 p-3 text-[10px] leading-relaxed text-slate-200">
                                  ...
                                  <ol className="mt-3 space-y-1.5 list-decimal pl-4">
                                    <li>「同梱カスタムノード5点・導入セットをダウンロード」を押して <code>ComfyUI_H3_FourPanel_NonLM_20260910101338_v5.9.7.zip</code> を保存します。</li>
                                    ...
                                    <li>下のボタンから <code>FourPanel_NonLM_4step_20260910101338_v5.9.7.json</code> を保存し、<code>ComfyUI/user/default/workflows/</code> 以下へ配置します。</li>
                                ...
                                <div className="mt-3 space-y-1 text-[10px] leading-relaxed text-slate-300">
                                  <p><strong>既定の設定</strong>：4コマを物語の4幕として扱い、台詞1本につき5秒で上限なしの完全可変尺にします。台詞がない場合だけ既定30秒です。映像はFused 4ステップ、音声補正は2ステップ（denoise 0.5）、既定はH3生成BGMありです。v5.9.7は区間ごとに生成・検査し、品質不合格なら初回込み最大5候補を比較します。途中で合格したら即座に次へ進み、全候補が不合格なら最良候補を保持して続行します。4・5回目の候補も途中再開できます。H3生成前には日本語台詞の読みも確認します。MiniMax H3が低音量の物語向けBGMを生成し、台詞中は強く抑えます。提供JSONの設定値は変更せずに配布します。</p>
                                  ...
                                  <p>人物集合と識別署名は各入力漫画の各コマから動的に導出し、特定の人数や外見、最終フレームの構成を固定しません。v5.9.7では人物IDと話者IDの対応を区間分割後も維持し、時系列の6フレームで話者の口の動きも検査します。全候補が不合格の場合、人物の重複などが検出されず、音声が合格で話者検査だけが不合格の候補を優先する場合があります。採用済みでも全検査合格とは限りません。完成動画の台詞と話者は利用者も確認してください。</p>
                                  <p>四コマ版の版番号はNano Bananaに合わせてv5.9.7へ訂正しました。別サービスの汎用動画版は更新しません。同梱の <code>VERSION_CORRECTION_JA.md</code>、<code>RELEASE_PREVENTION_JA.md</code>、<code>VALIDATION.md</code> に訂正内容、再発防止策、検証範囲を記載しています。</p>
