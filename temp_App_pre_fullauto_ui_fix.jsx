...
const SYSTEM_VERSION = "v2.82 Alpha";
      ...
      <div className="relative max-w-5xl mx-auto p-4 md:p-10 space-y-8">
        ...
        <header className="flex flex-col md:flex-row items-center justify-between gap-6 bg-[#0f1115] backdrop-blur-xl p-8 rounded-xl border border-white/5 shadow-2xl relative overflow-hidden group">
          ...
          <div className="flex flex-row items-center justify-center gap-4 z-10">
            <button onClick={resetAll} className="group flex items-center gap-2 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white px-8 py-4 rounded-2xl font-bold transition-all border border-white/5">
              <RefreshCw size={18} className="group-hover:rotate-180 transition-transform duration-500" /> 入力をリセット
            </button>
            <button
              onClick={handleFullAutoToggle}
              disabled={!apiKey}
              className={`group flex items-center gap-2 px-8 py-4 rounded-2xl font-bold transition-all border z-10 ${
                isFullAutoMode
              ...
              } ${!apiKey ? 'opacity-30 cursor-not-allowed' : ''}`}
            >
              {isFullAutoMode ? (
                <><Square size={18} className="animate-pulse" /> フルオート停止</>
              ) : (
                <><Zap size={18} className="group-hover:animate-bounce" /> フルオート</>
              )}
            </button>
          </div>
          {/* フルオート説明テキスト */}
          {isFullAutoMode && (
            <p className="text-[10px] text-emerald-400/70 text-center mt-1 animate-pulse">
              {fullAutoStep > 0 ? `⚡ STEP ${fullAutoStep} 実行中...` : '🚀 キャラクターシートをドロップしてください'}
            </p>
          )}
        </header>
        ...
        <div className="sticky top-4 z-50 bg-[#0f1115]/90 backdrop-blur-2xl border border-white/10 p-4 rounded-xl shadow-2xl flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-4">
            {/* フルオート実行中バッジ */}
            {isFullAutoMode && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[10px] font-bold tracking-wider bg-emerald-500/10 border-emerald-500/30 text-emerald-400 animate-pulse">
                <Zap size={12} />
                {fullAutoStep > 0 ? `AUTO STEP ${fullAutoStep}` : 'AUTO ARMED'}
              </div>
            )}
            {/* [v2.48] API接続状態バッジ */}
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-black italic tracking-tighter text-white">
                {isFullAutoMode && fullAutoStep > 0 ? (
                  <span className="text-emerald-400">FULL AUTO — STEP {fullAutoStep}</span>
                ) : (
                  <>
