...
- **Visible post-image quality failsafe / 可視画像品質フェイルセーフ:** STEP4 saves and displays the first provider image before reviewing attached character-sheet identity/outfit, four-panel count/order where applicable, anatomy, hand side, prop ownership, functional prop orientation, bubble text, speaker-name leakage, and extra text. The generated candidate is compared directly with the character sheets during QA; this does not add text to the image-generation prompt. A concrete NG triggers only one visible, issue-limited repair candidate. A passing repair replaces the original; if the repair remains NG or its request fails, the saved original image is restored and downstream work continues with a warning. An unverified QA result does not spend another image call. / STEP4は最初のAPI画像を保存・表示してから、添付キャラクターシートとの人物・服装一致、四コマ経路のコマ数・順序、解剖学、手の左右、小物の所有、機能面の向き、吹き出し本文、話者名混入、余計な文字を検査します。QAでは生成候補とキャラクターシートを画像同士で直接比較し、画像生成プロンプトの文字数は増やしません。具体的なNGがある場合だけ、問題箇所に限定した修正版を1回生成します。修正版が合格すれば置換し、修正版もNGまたは取得失敗なら保存済みの元画像へ戻して、警告付きで後続作業を続行します。QA判定不能時は追加の画像API呼び出しを行いません。
...
This route distributes `Super-FURU-AI-4koma-H3-FusedTurbo-Spectrum-SLA-8step-v1.json` and `MiniMax-H3-4Koma-15s-FusedTurbo-Spectrum-SLA-Bundle-2026-09-03.zip` as separate direct downloads. The bundle contains the four required custom-node packs, setup notes, model URLs and SHA256 values, and the supplied verification record. Defaults are 15 seconds, 16:9, 0.4 MP, 24 fps, Fused Turbo INT8 ConvRot, Spectrum degree 1, SLA 0.90 with block 64 and audio protection, RES Multistep, Simple, and 8 steps. Separate Turbo LoRA and SLA LoRA files are not required. / この経路では `Super-FURU-AI-4koma-H3-FusedTurbo-Spectrum-SLA-8step-v1.json` と `MiniMax-H3-4Koma-15s-FusedTurbo-Spectrum-SLA-Bundle-2026-09-03.zip` を別々に配布します。配布セットには必須カスタムノード4点、導入説明、モデルURL・SHA256一覧、提供元の動作確認記録を収録しています。既定は15秒・16:9・0.4MP・24fps・Fused Turbo INT8 ConvRot・Spectrum degree 1・SLA 0.90（block 64、audio protection有効）・RES Multistep・Simple・8 stepsです。独立したTurbo LoRAとSLA LoRAは不要です。
...
Each required loader embeds the supplied filename, download URL, SHA256, and ComfyUI target folder for the Fused Turbo diffusion model, Qwen3VL text encoder, `minimax_h3_video_vae_int8_convrot.safetensors`, and audio VAE. When you open the workflow, ComfyUI can show `Download` for the four registered missing models instead of only reporting an unresolved filename. / 必要な各ローダーには、Fused Turbo拡散モデル、Qwen3VLテキストエンコーダ、映像VAE `minimax_h3_video_vae_int8_convrot.safetensors`、音声VAEのファイル名、ダウンロードURL、SHA256、ComfyUI保存先を登録しています。ワークフローを開くと、ComfyUIは登録済みの不足4モデルを `Download` として表示できます。

Click “必須カスタムノード4点・導入セットをダウンロード,” extract `MiniMax-H3-4Koma-15s-FusedTurbo-Spectrum-SLA-Bundle-2026-09-03.zip`, then extract the four ZIP files under `02_カスタムノード`. Place each generated top-level folder under `ComfyUI/custom_nodes/`. If an older folder with the same name exists, fully close ComfyUI, back it up, and replace it as a folder; never mix old and new custom-node files. Save `Super-FURU-AI-4koma-H3-FusedTurbo-Spectrum-SLA-8step-v1.json` under `ComfyUI/user/default/workflows/`, then fully restart ComfyUI before opening it. / 「必須カスタムノード4点・導入セットをダウンロード」を押して `MiniMax-H3-4Koma-15s-FusedTurbo-Spectrum-SLA-Bundle-2026-09-03.zip` を保存します。配布セットを展開し、`02_カスタムノード` 内の4つのZIPをそれぞれ展開して、各最上位フォルダを `ComfyUI/custom_nodes/` へ配置します。同名の旧版がある場合はComfyUIを完全終了し、旧版をバックアップしてからフォルダ単位で差し替えます。新旧カスタムノードのファイルは混在させません。`Super-FURU-AI-4koma-H3-FusedTurbo-Spectrum-SLA-8step-v1.json` は `ComfyUI/user/default/workflows/` 以下へ保存し、開く前にComfyUIを完全に再起動してください。

The bundle supplies `NanoBananaH3Transform`, `SpectrumApplyMiniMaxH3`, `H3SLAAttention`, and `MiniMaxH3FastVAEDecode` through their corresponding four packages. / 配布セットの4パックは、`NanoBananaH3Transform`、`SpectrumApplyMiniMaxH3`、`H3SLAAttention`、`MiniMaxH3FastVAEDecode` をそれぞれ提供します。
