...
- **Three-attempt scenario quality selection / シナリオ品質は全3試行から選抜:** STEP2 evaluates scenario-content hygiene, manual-input exclusions, event-specific visual evidence, lightweight location/time/weather continuity, final-panel staging, and the speech-bubble dialogue contract. A full pass is accepted immediately. Ordinary quality targets retain the highest-scoring usable candidate with a concrete warning after three attempts and remain available to STEP3 and STEP4. The dialogue contract is blocking: if a candidate cannot be compiled into at least one bubble per panel, STEP2 retries it up to three total attempts; after exhaustion it never sends a contradictory no-dialogue prompt to STEP3 or STEP4 and tells the user to run scenario generation again. / STEP2では、本文の表現衛生、自由入力の禁止条件、出来事を示す視覚証拠、軽量な場所・時刻・天候の継続、4コマ目の能動演出、吹き出しセリフ契約を検証します。合格した時点で採用します。通常の品質条件は全3試行でも残る場合、最高得点の利用可能な候補を警告付きで保持してSTEP3・STEP4へ進めます。一方、各コマから最低1つの吹き出しを組み立てられない台詞契約は停止条件です。STEP2で合計3回まで自動再生成し、上限後も不成立なら矛盾した無台詞プロンプトをSTEP3・STEP4へ渡さず、利用者にシナリオ生成の再実行を案内します。
...
- **Provider routes / API経路:** OpenAI uses `gpt-4.1` for primary text/vision and offers GPT Image 2.0 / high, GPT Image 2.5 Flare / high or xhigh, and Sunburst / high or xhigh for 1024x1536 PNG generation. When an OpenAI key is verified, the app selects Sunburst / xhigh only when the returned model list includes Sunburst; otherwise it selects GPT Image 2.0 / high. This list-based default does not guarantee an image request or a quality setting will succeed, and selection lasts until reload. Gemini remains `gemini-3.5-flash` for primary text/vision and `gemini-3.1-flash-image` for manga generation. / STEP4では2.0 / highと、2.5 Flare・Sunburstそれぞれのhigh・xhighを選択できます。OpenAI APIキーの接続確認時、返却されたモデル一覧にSunburstがある場合だけSunburst / xhighを初期選択し、それ以外はGPT Image 2.0 / highを初期選択します。この一覧による初期選択は、画像生成や品質設定の成功を保証するものではありません。選択値はリロードまで保持します。2.0の利用もアカウント権限に依存します。
