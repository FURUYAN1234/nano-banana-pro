...
カメラの高低差・傾き・強い遠近感、全身の誇張、コマ間の画角・照明・効果の落差を積極的に許す指示を、シナリオ生成・演出強化・両プロバイダの画像用プロンプトへ適用します。１枚絵も共通の演技・カメラ指示を使います。人物・セリフ・小道具、明示カメラ・動作・静かな間は保持し、画像精査と修正候補の比較でも演出の大きさ自体を欠陥扱いしません。プロンプト制御の変更であり、生成画像の改善を保証するものではありません。

会社の同僚など、舞台に自然な背景人物は許容します。背景に人がいるだけで不合格にせず、メインキャラの髪・顔・眼鏡などが明らかに重複した増殖と区別します。明示された無人の場面は保持します。

通常のシナリオ生成では、動きの頂点に全身が入る画角を設計し、寄りや静かな間との落差を作ります。シナリオ強化で「身体」と「カメラ」を同時に選ぶと、画角と腰・膝・支持脚の動きを一緒に設計します。画像用の指示には、俯瞰で見える頭頂・机の天面や、低い位置から見上げる身体の軸など、指定ショットの見た目を具体化した補助指示を残します。明示された寄りや静かな場面は保持します。
...
人物と小物の前後関係・接触・輪郭のつながり、および表紙・背・紙面などの実際の面と文字の向き・遠近法を共通ルールで指定します。4コマの両プロバイダと1枚絵コピー用にも適用します。生成前精査では、既存の構図・物体の向きの補助行に安全に補足できる場合だけ、重なり、印刷面、実際の読み手・カメラ・表裏の関係を具体化します。人物へ向けた資料を反対側から見る場合の逆向きの文字や裏面、内容を見せる場合の読み手の背中・肩越し構図を指示します。文字だけを読者側へ正立させず、物体の向きと印刷面を保ちながら、選んだカメラの高低・傾き・強い遠近感から見える面を求めます。会話しているという理由だけで肩越しへ固定しません。横書き・縦書きを一律に制限せず、脚本で意図したシュールな現象と明示カメラは保ちます。Gemini向けのレンズ変換でも元のカメラ指定文を保持します。

本や資料の印字は、文字列の並びと一字ごとの上下を物体に固定し、物体と一緒に回転・透視投影するよう指定します。生成前精査の表裏補足は、同じコマのActionから引用した物体・読み手の情報を使い、アプリが条件付きの規則文を組み立てます。AIが自由文で書いた表裏の指示は直接採用しません。適用できない提案は原文を保持し、適用できる提案で続行します。

API画像検査では、各コマ（1枚絵では1場面）の物体境界、印刷面、読み手とカメラに対する表裏を個別に確認させます。画面の内容・印字・背面の筐体などの見える根拠と判定が矛盾する場合は未確認とし、その矛盾だけを理由に修正画像を生成しません。確認が欠けた回答もPASSにせず元画像を保持します。これは回答内の整合性の検証であり、ピクセルの認識そのものや文字方向の正しさを保証しません。Web貼り付け用にも生成前のルールは含まれますが、Webで完成した画像はアプリに自動で戻らないため、この生成後検査は実行されません。

Print layout and individual glyph orientation are fixed to the object's coordinates and rotate/project together. Functional-surface review uses same-panel Action evidence and application-compiled conditional rules instead of free-form projection advice. API QA records visible surface cues separately from expectations; unsupported or contradictory orientation judgments remain unverified and do not alone trigger image repair. This validates response consistency, not the accuracy of vision recognition.

Shared rules cover object/body contours, depth and contact, plus text alignment to the actual printed face, in both four-panel providers and the single-image copy prompt. They preserve reader-facing surfaces, inverted text from the opposite side of a table, and the actual reader's rear/shoulder viewpoint when front content needs to be shown. Gemini lens conversion preserves the literal camera direction; Web compaction retains named gaze targets and rear-shoulder owners. API QA requires separate boundary, printed-surface and reader/camera/front-back evidence for every scene; missing evidence retains the image as unverified without an extra image request. Web-paste outputs do not automatically return to the app for this post-generation QA. These checks can still miss visual defects.

Web貼り付け用の指示文にも、各コマを一つの瞬間として描くこと、後のコマの結果を先に見せないこと、脚本に沿った視線・反応対象、小道具の状態・持ち主の変化、キャラシートの配置やラベルを描かないことを含めます。脚本・台詞を優先し、品質上の懸念で出力を止める仕組みは追加しません。これらは生成モデルへの指示であり、誤りを必ず防ぐ機能ではありません。

STEP4の「品質検査NG時に自動修正する」は既定ONです。通常修正は1回です。同じ装飾的な印字の問題が修正後も検出された場合だけ、読めない印刷の質感へ置き換える最終候補をさらに1回生成します（合計最大2回の追加画像生成・追加課金）。対象は、正確な文言の指定や物語上の役割がないと検査で確認された印字だけです。台詞・タイトル・透かし・オチや手掛かりに必要な文字は保持します。各候補を比較し、最終候補が悪化した場合や失敗時も保持中の画像で続行します。検査の見落としや役割の判定不能ではこの最終処理は発動しないため、修正成功を保証しません。OFFでは追加画像を生成しません。/ Automatic repair is ON by default: one ordinary repair, then at most one conditional incidental-print fallback when the same grounded decorative text defect persists. This means at most two additional paid images. Dialogue, title, watermarks, exact requested text and plot-critical print remain protected. Compare candidates and retain the best available result even if defects remain. QA may miss defects or fail to establish text roles; OFF makes no additional image requests.

無言のコマは、STEP2のシナリオ内に独立した行で `セリフなし` と書くとSTEP3へ進めます。セリフの記入漏れやコマ不足は品質警告として知らせ、取得済みのシナリオを保持してプロンプト出力・STEP4を続行します。STEP2の品質修正が上限に達しても、実際に得た最良候補を警告付きで渡します。画像検査には手動編集を含む最終プロンプト全文を渡し、タイトル・セリフ／無言・左右の手・小道具の確認内容をログに表示します。確認内容が欠けた合格回答は「未確認」として元画像を保持し、その理由だけで追加画像生成は行いません。AI検査による誤判定や見落としは残るため、ログと実画像を照合してください。
STEP4は「API画像生成の品質」の直下に「API画像サイズ」を分けて表示します。サイズは標準1024×1536と大きめ1536×2304を選べます。既定はSunburst / xhigh・1024×1536です（Sunburstが利用可能でない場合は従来どおり2.0 / highを初期選択）。ユーザーが選んだ品質とサイズはリロードまで保持し、Webへコピーするプロンプトには反映しません。大きい解像度やmax品質が台詞・手・構図の正確さを保証するわけではありません。
...
- **Fail-closed cinematic technique library / 条件付き映像テクニック:** A bounded library can add at most two physically supported depth, shadow, motion, focus, reflection, refraction, or miniature-scale cues across a four-panel page, and at most one cue in the single-image copy prompt. Explicit camera, cast, action, anatomy, props, and readable text always win. Unsupported or conflicting scenes retain the exact baseline four-panel prompt, while every accepted addition replaces an existing fixed-length slot so the prompt does not grow. / 物理的な根拠がある場面だけ、奥行き、影、動感、焦点、反射、屈折、ミニチュア感の候補から4コマ全体で最大2件、一枚絵コピープロンプトで最大1件を条件付き適用します。明示カメラ、人物、動作、解剖、小道具、可読文字を常に優先し、根拠不足または競合時の4コマは従来プロンプトを完全維持します。採用時も既存の固定長スロット内だけを置換するため、プロンプトは長くなりません。
- **Body-acting and gesture variety / 身体演技・ジェスチャーの多様化:** Reference sheets define identity, not recurring poses. Story-motivated pointing, reaching, impact, full-body recoil and leaps are allowed across panels; a broad gesture-family count no longer triggers scenario retries. Vary silhouette, purpose and amplitude while preserving explicit actions, quiet beats, limb connections and prop ownership/facing. / 参照画像は人物の同一性資料として使い、ポーズを固定しません。指差し・手の突き出し・机への打撃をまとめて回数制限する判定を廃止し、全身の誇張、のけぞり、跳躍を場面に応じて許可します。身体の軸・目的・動作の大きさで変化をつけ、明示Action、静かな間、手足の接続と小道具の所有・向きを保ちます。
- **Bounded facial acting / 境界付き顔演技:** In the four-panel route, STEP2 turns each readable main face into a natural physical description selected from brow, eyelid/eye opening, gaze target, mouth shape, and head/torso angle; the optional expression enhancement uses the same contract. STEP3 renders those cues with bold or subtle expressions appropriate to the scene, without forcing close-ups, front-facing poses, or camera gaze, and never prints acting labels. Scripted camera, action, conversational eye-line, hands, and props remain higher priority. This prompt guidance can improve expression specificity, but the image model still determines the final pixels and exact reproduction is not guaranteed. / 4コマ経路では、STEP2が顔を読み取れる主要人物について、眉、まぶた・目の開き、視線の相手、口の形、頭・胴体の傾きから場面に必要な要素を選び、自然な物理描写へ変換します。任意の「表情」強化も同じ契約を使います。STEP3は場面に合った大胆な表情と繊細な表情の両方を許し、寄り・正面・カメラ目線を強制せずに描画へ渡し、演技ラベルを印字しません。指定カメラ、動作、対人視線、手、小道具を優先します。表情の具体性を導くプロンプト制御であり、最終ピクセルは画像モデルに依存するため完全再現は保証しません。
- **Three-attempt scenario quality selection / シナリオ品質は全3試行から選抜:** STEP2 evaluates scenario-content hygiene, manual-input exclusions, event-specific visual evidence, lightweight location/time/weather continuity, final-panel staging, and the speech-bubble dialogue contract. A full pass is accepted immediately. Ordinary quality targets retain the highest-scoring usable candidate with a concrete warning after three attempts and remain available to STEP3 and STEP4. Dialogue-quality failures also retain the best actual candidate with warnings after the retry budget; STEP3 and STEP4 continue. Explicit silent panels do not require speech. / STEP2では、本文の表現衛生、自由入力の禁止条件、出来事を示す視覚証拠、軽量な場所・時刻・天候の継続、4コマ目の能動演出、吹き出しセリフ契約を検証します。合格した時点で採用します。通常の品質条件は全3試行でも残る場合、最高得点の利用可能な候補を警告付きで保持してSTEP3・STEP4へ進めます。台詞の品質不足も合計3回までの修正後は最良の取得済み候補を警告付きで保持し、STEP3・STEP4を続行します。意図した無言のコマに台詞は要求しません。
...
- **Dialogue and visual-direction boundary / 台詞と視覚指示の境界:** Only quoted dialogue is sent to speech bubbles. Every generated panel must provide at least one standalone bubble-compatible line such as `キャラ名「短いセリフ。」`; a quote embedded in `状況`/Action or another visual-direction line remains visual direction and cannot satisfy the dialogue contract. When one line contains multiple explicitly attributed speakers, each quoted utterance is separated and assigned to its own speaker; surrounding directions such as speaking softly, laughing, or replying sharply remain non-printing stage metadata. Markdown headings and Action/visual directions remain drawing instructions, including combined acoustic-direction labels such as `効果音・演出`, so expression, body, staging, sound, and background notes are not rendered as extra bubble text or synthetic speakers. Age-up normalization applies to standalone person roles and does not rewrite compound words such as `青少年` inside verbatim dialogue. / 引用符内の台詞だけを吹き出しへ渡します。生成する各コマは `キャラ名「短いセリフ。」` のような吹き出し化可能な独立行を最低1つ持つ必要があります。`状況`・Actionなど視覚指示の行に埋め込まれた発話は視覚指示のままであり、台詞契約を満たしません。1行に明示話者が複数並ぶ場合も、各引用台詞を話者ごとに分離し、「小声で」「笑う」「鋭く返す」など引用外の演出説明は印字しません。Markdown見出しやAction・視覚指示に加え、`効果音・演出` のような音響・演出の複合ラベルも描画指示のまま保持し、表情・身体・演出・音響・背景の説明を余計な吹き出し文字や架空話者として描画しません。成人化変換は独立した人物属性だけに適用し、原文台詞内の `青少年` などの複合語を部分置換しません。
- **Four-panel type hierarchy / 4コマの文字階層:** The top-page title uses an extra-bold, condensed Japanese Gothic treatment, while every white speech bubble uses the same slender, regular-weight Mincho-style vertical Japanese treatment. Dialogue keeps that bubble treatment even for a shout or punchline; emphasis comes from the bubble and composition rather than switching to bold Gothic. This is a rendering instruction, so the image model still determines final pixels and exact font-file identity is not guaranteed. / ページ上部の見出しは極太・コンデンス寄りの和文ゴシック、白い吹き出し内の台詞は細め・標準ウェイトの明朝寄り縦組みで統一します。叫びやオチでも吹き出しの台詞を太ゴシックへ切り替えず、強調は吹き出し形状と構図で表します。これは生成指示であり、最終ピクセルおよび特定フォントファイルの完全一致は画像モデルが決めるため保証しません。
...
- **Visible post-image quality failsafe / 可視画像品質フェイルセーフ:** STEP4 preserves the first image, checks it, and allows one ordinary repair. A persistent, grounded incidental-print defect may trigger one final texture-only lettering fallback; protected story text stays readable. Candidates are compared, ties or failed comparisons retain the earlier candidate, and remaining defects do not stop output. OFF or unverified initial QA makes no additional image request. / 元画像を保存して検査し、通常修正は1回、同じ装飾印字の問題が残る場合だけ最終候補をさらに1回生成します。台詞・タイトル・物語に必要な文字は保持します。候補を比較し、同等・比較失敗なら前の候補を保持し、未解消でも画像を出して続行します。画像生成・画像認識にはAPI料金がかかり、検査の完全性は保証しません。
- **Provider routes / API経路:** OpenAI uses `gpt-4.1` for primary text/vision and offers GPT Image 2.0 / high, GPT Image 2.5 Flare / high or xhigh, and Sunburst / high, xhigh or max. Output size is independently selectable as 1024x1536 or 1536x2304 PNG. When an OpenAI key is verified, the app selects Sunburst / xhigh only when the returned model list includes Sunburst; otherwise it selects GPT Image 2.0 / high. This list-based default does not guarantee an image request or a quality setting will succeed, and selection lasts until reload. Gemini remains `gemini-3.5-flash` for primary text/vision and `gemini-3.1-flash-image` for manga generation. / STEP4では2.0 / high、2.5 Flareのhigh・xhigh、Sunburstのhigh・xhigh・maxを選択できます。直下のAPI画像サイズ欄で1024×1536または1536×2304を独立して選べます。OpenAI APIキーの接続確認時、返却されたモデル一覧にSunburstがある場合だけSunburst / xhighを初期選択し、それ以外はGPT Image 2.0 / highを初期選択します。この一覧による初期選択は、画像生成や品質設定の成功を保証するものではありません。選択値はリロードまで保持します。2.0の利用もアカウント権限に依存します。
- **Organization verification / 組織認証:** GPT Image 2.5 may require organization verification. Open [organization settings](https://platform.openai.com/settings/organization/general), choose Verifications → Individual → Start, and complete Persona identity verification using an accepted document in the official UI. Wait for approval and API propagation. An explicit verification-required API error displays a warning beside the selector; choose another model manually if desired. No automatic model switch or retry occurs after an image-generation error, and access to GPT Image 2.0 is not guaranteed. / 本人確認書類は公式画面だけで提出してください。明示的な未認証エラー時にはプルダウンの横に認証・モデル変更案内が表示されます。画像生成エラー後の自動切替・自動再送信は行いません。一般の権限エラーや通信障害を未認証とは判定しません。
...
When automatic repair is enabled and QA identifies a concrete problem, the OpenAI route submits the completed original image plus the same reference images for one targeted edit. Every completed repair is directly compared, including one with remaining NG or unresolved QA. A clear improvement is retained with any remaining warning; ties or failed comparisons retain the original. If the same confirmed incidental-print defect remains, one final print-texture candidate may be generated. Each candidate consumes an additional image API call. An initially unverified review alone does not trigger repair.

自動修正がONで品質検査が具体的な問題を検出した場合、OpenAI経路では完成した元画像と同じ参照画像を渡して通常1回限定修復します。修復画像にNGや判定不能が残っても直接比較し、明確な改善を確認した場合は警告付きで採用します。同等・比較失敗なら元画像を保持します。同じ装飾印字の不具合が残る場合だけ、印字を簡略化する最終候補を1回追加できます。各候補には画像API料金がかかります。初回検査が判定不能だけの場合は修復しません。
...
| STEP4 Image generation / 画像生成 | Current prompt and character sheets; enabled panorama for OpenAI or four background crops for Gemini / 現在のプロンプトとキャラシート。OpenAIは有効パノラマ、Geminiは有効な4枚の背景切り出し画像も添付 | OpenAI uses images/edits with references, or the existing text-only generation route without them. Edits require a completed image and never automatically resend. Gemini adds character sheets before its existing explicit references or background crops. Saves the original and runs QA; a concrete NG may trigger one ordinary repair, then one conditional incidental-print fallback. OpenAI edits attach the selected source candidate first, then the same references. Only text-only OpenAI generation retains its legacy partial-image recovery and one non-streaming retry after browser-level Failed to fetch. / OpenAIは参照ありなら画像編集API、参照なしなら従来の文章生成経路を使います。画像編集は完成画像を必須とし、自動再送しません。Geminiは既存の明示参照・背景cropの前にキャラシートを添付します。元画像を保存してQAを行い、具体的NG時に通常1回修復し、同じ装飾印字NGが残る場合だけ最終候補を1回追加します。OpenAI修復には選択した元候補を先頭に、同じ参照を続けて添付します。途中画像救済と通信失敗後の非ストリーム再試行は、参照なしOpenAI生成の既存経路だけに残ります。 | Compare every completed repair against the retained image, including candidates with unresolved or failed QA. Keep an improved candidate with a warning; retain the earlier image on ties or failed comparison. / 通常修復も最終候補も、判定不能・NGが残る場合を含めて保持画像と直接比較します。改善した候補を警告付きで採用し、同等・比較失敗なら先の画像を保持します。 |
...
| Image received but visible QA is NG / 画像受信後の可視QAがNG | Keep the original and try one ordinary repair for concrete issues. Only if the same confirmed incidental-print defect remains, try one final illegible-print candidate and compare available images. Preserve dialogue and story-required text; keep the best available image with a warning if QA remains NG. / 元画像を保持して具体的な問題を通常1回修復します。同じ装飾印字の問題が確認されて残る場合だけ、読めない印字の最終候補を1回追加して比較します。台詞と物語に必要な文字は保護し、NGが残っても最良の画像を警告付きで保持します。 |
...
### v6.0.0 (2026-09-11)
- **[Fix & UX]** ギャグを保つ任意のAI精査と変更前後・復元を追加。品質警告でも出力を継続し、無言コマ、脚本優先の補助指示、API品質と画像サイズの独立選択を改善。 / Added optional comedy-preserving AI review with before/after and undo, continued output on quality warnings, silent panels, script-first directions, and independent API quality and size selection.
...
- GPT Image 2.5 Flare/Sunburst high・xhighと2.0の選択、個人認証案内、リロードまでの選択保持を追加。実API比較からSunburst / xhighを初期値に設定。/ Adds five image choices and verification guidance, with a session-only Sunburst / xhigh default based on a bounded live comparison.
- 自動修正は既定ON。修正版が品質検査を通過しても元画像と直接比較し、改善を確認できた場合だけ採用。同等・比較失敗では元画像を保持。/ Automatic repair defaults on; a passing repair is adopted only when direct comparison confirms improvement, otherwise the original is retained.
- Action内の無言の登場人物が人数指定から抜ける不具合を修正。Web貼り付けプロンプトとAPI設定を仕切り線で区別。/ Fixes silent Action actors omitted from cast constraints and separates API controls from Web-copy prompts.
...
### v5.8.4 (2026-09-06)
- **[Distribution]** Fused4step＋SLA版MiniMax H3配布セットを更新し、ZIP内手順書をH3生成BGMあり・不足モデルのダウンロード候補・他PC導入手順へ同期 / Updated the Fused4step + SLA MiniMax H3 distribution and synchronized its bundled guide with H3-generated BGM, missing-model download candidates, and cross-PC setup steps
...
### v5.8.1 (2026-09-04)
- **[Fix & UX]** 最新のH3音声2ステップ配布ワークフローと導入セットへ更新 / Updated to the latest H3 audio two-step distribution workflow and setup bundle
...
### v5.7.4 (2026-09-03)
- **[Fix & UX]** MiniMax H3配布を実行時API入力ゲート版へ更新し、未登録時のキュー保留、認証後の同一実行再開、中止時の安全なfalse返却、全配布文書を同期 / Updated the MiniMax H3 distribution with an execution-time API credential gate, queue hold for unregistered providers, same-run resume after verification, safe false cancellation, and synchronized documentation

### v5.7.3 (2026-09-03)
- **[Fix & UX]** ComfyUIカスタムノードのAPIキーをディスク保存からプロセスメモリ限定へ変更し、MiniMax H3配布の最重要資格情報ゲート、ワークフロー作成PC用手順、全配布文書を同期。v5.7.3の読込時ダイアログ動作はv5.7.4の実行時ゲートへ置き換えました / Changed the ComfyUI custom node from disk-persisted API keys to process-memory-only credentials and synchronized the highest-priority MiniMax H3 credential gate, workflow-author-PC guide, and distribution documentation. v5.7.4 supersedes v5.7.3's load-time dialog behavior with an execution-time gate
...
### v5.6.8 (2026-08-28)
- **[Fix & UX]** MiniMax H3の最新版・最強版配布ワークフローをユーザー提供のHybrid b25 Turbo v4 LoRA 8-step JSONへ差し替え / Replaced the latest MiniMax H3 workflow download with the user-provided Hybrid b25 Turbo v4 LoRA 8-step JSON

### v5.6.7 (2026-08-28)
- **[Fix & UX]** 条件付き映像テクニックとキャラクターシート参照QAを追加し、固定長プロンプトを維持 / Adds conditional cinematic techniques and character-sheet QA while preserving fixed prompt length
...
### 画像QAの観察根拠と候補選択 / Image QA evidence and selection

人物の外見違いは、コマ内の位置・検査対象以外の識別特徴2点・参照画像の根拠・実際に見える特徴を揃えて判定します。印字は背景の本も含め、物体の軸と文字の軸を別々に記録し、根拠不明の向きを合格扱いしません。通常修正版にNGや判定不能が残っても直接比較し、改善が確認できた候補を警告付きで保持します。同等・比較失敗では先の画像を残します。これらはAI判定の内部整合を確認する処理であり、画素の正確な認識を保証しません。

Character mismatch claims require localized identity and reference evidence. Printed props, including background books, record object and glyph axes separately. Every completed repair is compared even with unresolved or failed QA; a demonstrably better candidate is retained with a warning, while ties or failed comparison retain the earlier image. These checks validate review consistency, not independent pixel recognition.
