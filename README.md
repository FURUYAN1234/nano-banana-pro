...
**Layer 4 — Punchline Diversity Engine (v2.90+) / 第4層 — オチ多様化エンジン（マンネリ防止）:**
**Layer 4 — オチ多様化エンジン（v2.90+）:**
...
The combined effect of both mechanisms realizes a principle analogous to theater casting: **"The actors remain the same; the genre of the script changes every time."** The persona defines *how* a character speaks and reacts, while the punchline type defines *what situation* they are placed in. When these two layers are decoupled, the same cast produces entirely different chemical reactions across generations — without ever breaking character.
両メカニズムの複合効果は、演劇のキャスティングに似た原則を実現します：**「役者は同じ、脚本のジャンルが毎回変わる」**。ペルソナはキャラクターの「喋り方・反応の味」を規定し、オチタイプは「どんなシチュエーションに置かれるか」を規定します。この2層が分離されると、同じキャストでも生成のたびに全く異なる化学反応が生まれ——キャラクターの個性を壊すことなく、無限のバリエーションが得られます。
