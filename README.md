...
人物と小物の前後関係・接触・輪郭のつながり、および表紙・背・紙面などの実際の面と文字の向き・遠近法を共通ルールで指定します。4コマの両プロバイダと1枚絵コピー用にも適用します。生成前精査では、既存の構図・物体の向きの補助行に安全に補足できる場合だけ、重なり、印刷面、実際の読み手・カメラ・表裏の関係を具体化します。人物へ向けた資料を反対側から見る場合の逆向きの文字や裏面、内容を見せる場合の読み手の背中・肩越し構図を指示します。文字だけを読者側へ正立させず、自動の構図多様化より物体の向きを優先します。横書き・縦書きを一律に制限せず、脚本で意図したシュールな現象と明示カメラは保ちます。Gemini向けのレンズ変換でも元のカメラ指定文を保持します。
...
- **Bounded facial acting / 境界付き顔演技:** In the four-panel route, STEP2 turns each readable main face into a natural physical description selected from brow, eyelid/eye opening, gaze target, mouth shape, and head/torso angle; the optional expression enhancement uses the same contract. STEP3 renders those cues without forcing close-ups, front-facing poses, or camera gaze, and never prints acting labels. Scripted camera, action, conversational eye-line, hands, and props remain higher priority. This prompt guidance can improve expression specificity, but the image model still determines the final pixels and exact reproduction is not guaranteed. / 4コマ経路では、STEP2が顔を読み取れる主要人物について、眉、まぶた・目の開き、視線の相手、口の形、頭・胴体の傾きから場面に必要な要素を選び、自然な物理描写へ変換します。任意の「表情」強化も同じ契約を使います。STEP3は寄り・正面・カメラ目線を強制せずに描画へ渡し、演技ラベルを印字しません。指定カメラ、動作、対人視線、手、小道具を優先します。表情の具体性を導くプロンプト制御であり、最終ピクセルは画像モデルに依存するため完全再現は保証しません。
