...
任意の「ギャグを保ってAI精査」は、接続中の文章APIで脚本・人物設定と補助指示の矛盾を確認します（追加料金・待ち時間あり）。シュールな出来事、感情のずれ、無反応、意図が曖昧な破綻は保持し、明確に競合する構図・視線などの補助指示だけを調整します。脚本・台詞・オチ自体は書き換えません。理由と変更前後を表示し、「精査前の指示文に戻す」で元に戻せます。失敗時は原文を保持し、精査中もコピー・画像生成は続けられます。精査結果はWeb貼り付けとAPI生成の両方で使えます。誤解釈や見落とし、画像の誤描画を必ず防ぐものではありません。後から本文を手動編集した場合、古い復元操作は表示しません。

Optional AI comedy review uses the connected text API, with additional cost and latency. It preserves surreal events and ambiguous intent, edits only conflicting auxiliary directions, shows reasons and before/after text, and supports undo. Script, dialogue and ending remain unchanged; failure retains the original and never blocks copying or image generation. This is prompt assistance, not a guarantee of image accuracy.
...
> Latest release: **v6.0.0** / 最新リリース: **v6.0.0**
...
The current implementation and latest release are **v6.0.0**. The product name is **Super FURU AI 4-koma System**. STEP4 offers Gemini and OpenAI image-generation routes, including GPT Image 2.5. / 現在の実装および最新公開版は **v6.0.0** です。製品名は **Super FURU AI 4-koma System** で、STEP4ではGPT Image 2.5を含むOpenAI系とGemini系の画像生成経路を利用できます。
...
- **Visible post-image quality failsafe / 可視画像品質フェイルセーフ:** STEP4 preserves the first image and checks cast, panels, anatomy, props and dialogue. With automatic repair enabled, concrete issues trigger at most one repair image. A passing repair is compared directly with the original using a vision request; it replaces the original only when clearly better. Ties, uncertain comparison, failed requests and failed repair QA retain the original with a warning. OFF or unverified initial QA never triggers a repair image. / 元画像を保存して人物・コマ・手・小物・台詞を検査します。自動修正ONで具体的なNGがある場合だけ追加画像を最大1回生成します。修正版が検査を通過した場合は元画像と画像認識APIで直接比較し、明確な改善がある場合だけ採用します。同等・判定不能・失敗時は元画像と警告を保持します。画像生成と比較用の画像認識にはAPI料金がかかります。自動検査は誤り得るため、台詞の完全一致を保証するものではありません。
