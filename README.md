...
> Latest release: **v5.3.2** / 最新リリース: **v5.3.2**
...
The current implementation is **v5.3.2**. The product name is **Super FURU AI 4-koma System**; **Nano Banana 2** and **ChatGPT Image 2.0** identify image-generation engine families. / 現在の実装は **v5.3.2** です。製品名は **Super FURU AI 4-koma System** で、**Nano Banana 2** と **ChatGPT Image 2.0** は画像生成エンジン系統の名称です。
...
### 🧭 Current v5.3.2 Processing Contract / 現行v5.3.2処理仕様
...
| STEP4 Image generation / 画像生成 | Current STEP3 prompt and character references / STEP3の現在のプロンプトとキャラクター参照 | Sends one image request to the selected provider. OpenAI uses streaming, retains the newest valid partial when available, and retries once without streaming only after a browser-level stream `Failed to fetch`. It then runs one visible quality review. / 選択したAPIへ画像生成を1回送信します。OpenAIはストリーミングを使い、利用可能なら最新の途中画像を保持し、ブラウザレベルのストリーム `Failed to fetch` の場合だけ通常応答で1回再試行します。その後、1回の可視品質レビューを実行します。 | Returned image remains displayed and downloadable; its review result is shown without automatic regeneration / 受信画像を表示・保存したまま、レビュー結果を表示。自動再生成はしない |
...
| Image successfully received / 画像受信成功 | Display the image, then run one visible review for people/hands, prop ownership and functional orientation, dialogue bubbles, speaker-name leakage, and unrelated text. Keep the image and never auto-regenerate after that review. / 画像を表示してから、人物・手、小物の所有と機能面の向き、吹き出し、話者名混入、無関係な文字を1回だけ可視レビューします。画像は残し、そのレビュー後に自動再生成はしません。 |
