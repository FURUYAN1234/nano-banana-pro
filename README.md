...
- **Provider routes / API経路:** OpenAI uses `gpt-4.1` for primary text/vision and offers GPT Image 2.0 / high, GPT Image 2.5 Flare / high or xhigh, and Sunburst / high or xhigh for 1024x1536 PNG generation. Sunburst / xhigh is the default, selected from a one-image-per-setting comparison, not a guarantee of universal superiority. Selection lasts until reload. Gemini remains `gemini-3.5-flash` for primary text/vision and `gemini-3.1-flash-image` for manga generation. / STEP4では2.0 / highと、2.5 Flare・Sunburstそれぞれのhigh・xhighを選択できます。初期値はSunburst / xhighです。各条件1枚の実API比較に基づく選定で、常に最良とは限りません。選択値はリロードまで保持します。個人認証が未完了・審査中の場合は2.0を手動選択してください。2.0の利用もアカウント権限に依存します。
- **Organization verification / 組織認証:** GPT Image 2.5 may require organization verification. Open [organization settings](https://platform.openai.com/settings/organization/general), choose Verifications → Individual → Start, and complete Persona identity verification using an accepted document in the official UI. Wait for approval and API propagation. An explicit verification-required API error displays a warning beside the selector; manually select GPT Image 2.0 if desired. No automatic model switch occurs, and access to 2.0 is not guaranteed. / 本人確認書類は公式画面だけで提出してください。明示的な未認証エラー時にはプルダウンの横に認証・2.0への変更案内が表示されます。自動切替・自動再送信は行いません。一般の権限エラーや通信障害を未認証とは判定しません。
