...
> Current source version: **v6.4.4** / 現在のソース版: **v6.4.4**
...
9. **Lock script and lettering / 台本・文字固定:** Keep four panels with content-driven heights, exact dialogue, speaker order and title. Reserve Japanese balloons from right to left independently of character placement: B1 is the rightmost body and later balloons move left. Tail roots leave the lower speaker-facing half of each bubble, use a clear route around heads/faces/hair/text, and end at the assigned speaker. Unspecified surfaces remain unlettered in the generation request, while QA tolerates plausible AI-completed environmental text unless its meaning materially contradicts the approved story. Image QA transcribes every visible glyph from pixels rather than inferring correctness from this prompt. / 内容に応じた高さの4コマ枠、台詞全文、話者順、タイトルを保持します。人物配置と独立して日本語吹き出しを右から左へ予約し、B1を最右、後続を左へ置きます。尻尾は吹き出し下半分の話者側から出し、頭・顔・髪・台詞文字を避けて指定話者へ接続します。生成指示では未指定面を無文字のままにしつつ、画像QAではAIが補完した自然な環境文字を、意味が承認済みの物語と大きく矛盾しない限り許容します。画像QAはプロンプトから正しさを推測せず、実画像に見える文字をすべて転記して照合します。
...
For OpenAI text, STEP2 scenario creation and its optional enhancement default to GPT-6 Astra. The STEP2 selector can pin a lower-cost starting model until reset; a failure only falls through to models below that selection: Astra → Sol → Terra → Luna → GPT-4.1 → GPT-4.1 mini → GPT-4.1 nano → GPT-4o. Each option includes a short scenario-quality/cost note plus the release-bundled input/output list-price reference (USD per million tokens). That reference is updated for releases, is not live billing, and excludes tool fees, cache discounts, taxes, and account-specific terms. The progress log reports both the fixed start and final adopted model. Character analysis, prompt review and image generation keep their existing routes. Model availability and a successful API response do not by themselves guarantee story quality; review the generated scenario before STEP3. / OpenAI文章処理では、STEP2のシナリオ作成と任意のシナリオ強化を既定でGPT-6 Astraに固定します。STEP2の選択欄から低コストの開始モデルへ切り替えるとリセットまで維持され、失敗時だけ選択位置より下位のAstra → Sol → Terra → Luna → GPT-4.1 → GPT-4.1 mini → GPT-4.1 nano → GPT-4oへ順に切り替わります。各選択肢にはシナリオ品質・コストの短い説明と、公開時点で同梱する入力／出力の参考単価（USD／100万トークン）を表示します。この単価表は公開ごとに更新し、リアルタイム請求額ではありません。ツール料金・キャッシュ割引・税・アカウント固有の条件は含みません。進捗ログには固定開始と最終採用モデルを表示します。キャラクター解析、プロンプト確認、画像生成の経路は従来どおりです。モデルが利用可能でAPI応答が成功しても物語の質までは保証しないため、STEP3の前に生成シナリオを確認してください。
...
For OpenAI image prompts, the bottom-right English watermark includes the model actually adopted for STEP2, for example `ChatGPT / GPT-5.6 Terra / FURU AI 4-koma v6.4.4`; the bottom-left Japanese watermark is unchanged. / OpenAI画像用の最終プロンプトでは、右下の英語透かしにSTEP2で実際に採用されたモデル名を入れます（例: `ChatGPT / GPT-5.6 Terra / FURU AI 4-koma v6.4.4`）。左下の日本語透かしは変更しません。
...
Web貼り付けとAPIで共通の元プロンプトに、B番号に対応する話者を保持します。複数の吹き出しは話者の位置と切り離して `RIGHTMOST` / `LEFT OF Bn` / `LEFTMOST` の順序を固定し、単独の吹き出しは話者側の空きへ置いて、頭・顔・髪・文字を横切らない最短のしっぽを指定話者へ接続します。Web短縮でも話者、順序、しっぽの接続先を削りません。古いプロンプトはSTEP3で再構築してください。ト書きの短縮名・背景集団からの人数制約と、編集済みの結末・自由記述の人物特徴も反映します。 / The shared API and Web prompt retains each B-number-to-speaker mapping. Multiple bubbles keep their explicit `RIGHTMOST` / `LEFT OF Bn` / `LEFTMOST` order independently of speaker position; a single bubble instead uses clear space on its speaker's side and the shortest tail route that avoids heads, faces, hair and text. Web compaction preserves the speaker, order and tail endpoint.

Page proportions are both generation targets and a conditional local normalization step. Standard output stays 1024×1536 with a 100px title band, one undivided 1402px four-panel block, and a compact 34px footer; the large API tier stays 1536×2304 with every value scaled by 1.5. Individual panel heights remain content-driven because the panel block is moved and scaled as one bitmap; the app never slices its four panels or replaces the model-rendered extra-bold condensed Japanese Gothic title. If the generated title is enclosed by an aligned rectangular rule, normalization removes that surrounding rule while retaining the rendered title glyphs. Side insets are 6px at standard size and 9px at large size. Normalization runs only when the title, whole panel block, and footer can be detected and fitted without cropping artwork or distorting its aspect ratio. It may trim blank page margin outside the detected outer frame, while preserving the frame, every panel, and every internal gutter. Otherwise the generated pixels are kept unchanged and the UI reports that normalization was skipped. The result uses the original single image-download control; no extra fixed-ratio, raw-image, or QA-recheck controls are added. Both watermark lines are restricted to the outer footer and must not be duplicated inside story panels or on in-scene pages. / ページ配分は生成時の目標に加え、条件付きのローカル正規化も行います。標準は1024×1536（タイトル100px、分割しない4コマ全体1402px、フッター34px、左右6px）、大サイズは1536×2304のまま全値を1.5倍して保持します。4コマ全体を1枚のビットマップとして移動・拡縮するため、各コマの高さは内容に応じた比率を維持し、個別コマの切断やモデルが描いた極太・長体の日本語ゴシックタイトルの置換は行いません。タイトルが上下左右の揃った矩形罫線で囲まれている場合は、文字画素を保持したまま周囲の罫線だけを正規化対象から外します。タイトル・4コマ全体・フッターを確定でき、絵を切らず縦横比も変えずに収まる場合だけ正規化します。検出した外枠より外側の白余白は除けますが、外枠線・全コマ・コマ間余白は丸ごと保持します。確定できない場合は生成画素を変更せず、未適用であることをUIに表示します。結果画面は従来の画像ダウンロードボタンだけを使い、固定比率、加工前画像、QA再検査の追加ボタンは表示しません。透かし2行は外側フッターだけに1回ずつ置き、コマ内や作中原稿へ複製しないよう指示します。
...
Image QA checks panel structure, character identity, body and hand integrity, prop ownership, dialogue-only bubbles, text leakage, and observable surface orientation. For every large, foreground, foreshortened, open, or action-critical hand, it inventories visible and naturally occluded digits and requires their total to be five. A four-digit hand is an anatomy defect; uncertain or missing digit evidence remains unverified. Missing evidence triggers one review of the same image, then remains unverified without image regeneration if unresolved. / 画像QAはコマ構造、人物同一性、身体・手、小物の持ち主、吹き出しが台詞だけか、余分な文字、確認可能な面の向きを検査します。大きい手、前景の手、短縮遠近の手、開いた手、動作上重要な手ごとに、見える指と自然に隠れた指を数え、合計5本であることを必須にします。4本指は人体不具合とし、判別不能・根拠不足は未確認に残します。根拠不足は同じ画像を1回再検査し、解消しなければ未確認のまま採用します。それだけを理由に画像を再生成しません。
