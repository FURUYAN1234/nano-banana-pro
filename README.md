...
2. **【描画担当】Gemini API (Nano Banana 2) & OpenAI API (gpt-image-2) (The Renderer)**
   * **Strength / 強み:** You have the choice of two top-tier rendering engines. The default Gemini API (Nano Banana 2 model) offers extremely high-speed, native generation within the Google ecosystem. The newly integrated OpenAI API (gpt-image-2) provides industry-leading artistic quality, accessible directly in-browser without copy-pasting. / 2つの最高峰レンダリングエンジンを選択可能です。デフォルトのGemini API（Nano Banana 2モデル）はGoogleエコシステム内での超高速・ネイティブな生成を提供し、OpenAI API（gpt-image-2）は業界最高水準の画質をコピペ不要でアプリ内から直接提供します。
   * **Role / 役割:** Acts as the "Artist." The Gemini-crafted prompt is sent to either the Gemini API or the OpenAI API (depending on your selection), which renders the final manga image with high fidelity. / 「作画者」として機能します。Geminiが構築したプロンプトは、ユーザーの選択に応じてGemini APIまたはOpenAI APIへ送信され、高品質なマンガ画像を出力します。
3. **【上級者向け】Browser UI Option (Advanced / Optional)**
   * For users seeking even higher quality through **reference image attachment** (a feature currently unavailable via standard APIs), the system supports manually copying the generated prompt into the **ChatGPT or Gemini browser UI** alongside character sheet images. This achieves near-perfect character consistency that API-only generation cannot match. / **参照画像の添付** （現在の標準APIでは利用不可な機能）によるさらなる高品質を求める上級者向けに、生成されたプロンプトをキャラクター設定画と共に **ChatGPT または Gemini のブラウザUI** へ手動コピーする運用もサポートしています。これにより、API単体では到達できない完璧に近いキャラクターの一貫性を実現します。
