...
test('HF deploy tracks distribution ZIPs through the Hub LFS bridge without changing Pages assets', async () => {
  ...
  const lfsTrackCommand = 'git lfs track "downloads/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06-r5.zip"';
  const lfsTrackIndex = script.indexOf(lfsTrackCommand);
  const workflowByteRule = '"workflows/Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06-r5.json -text"';
  const checksumByteRule = '"downloads/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06-r5.zip.sha256.txt -text"';
