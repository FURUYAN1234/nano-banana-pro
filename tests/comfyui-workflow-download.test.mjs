...
const workflowUrl = new URL('../public/workflows/Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06.json', import.meta.url);
const customNodeZipUrl = new URL('../public/downloads/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06.zip', import.meta.url);
const customNodeZipShaUrl = new URL('../public/downloads/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06.zip.sha256.txt', import.meta.url);
...
test('STEP4 provides separate Fused4step SLA workflow and three-node bundle downloads', () => {
  assert.match(step4PanelSource, /MiniMax H3・ComfyUI用プロンプトをコピー[\s\S]*必須カスタムノード3点・導入セットをダウンロード[\s\S]*Fused4step・SLA ワークフローをダウンロード/);
  ...
  assert.match(step4PanelSource, /Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06\.json/);
  assert.match(step4PanelSource, /MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06\.zip/);
  ...
  assert.match(step4PanelSource, /H3 SLA Attention.*ComfyUI-PlagueKind-Nodes.*Triton/s);
...
test('supplied Fused4step SLA workflow bytes and graph are preserved', () => {
  ...
  assert.equal(hashBytes(bytes), '1e26382f82eae197b45295d41317d70cff676b27ef1be1390098c60447b97576');
  assert.match(readFileSync(sourceAttributesUrl, 'utf8'), /public\/downloads\/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06\.zip -text/);
  assert.match(readFileSync(sourceAttributesUrl, 'utf8'), /public\/downloads\/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06\.zip\.sha256\.txt -text/);
  assert.match(readFileSync(publishedAttributesUrl, 'utf8'), /Super-FURU-AI-4koma-H3-Hybrid-b25-Fused4Step-SLA-2026-09-06\.json -text/);
  assert.match(readFileSync(publishedAttributesUrl, 'utf8'), /downloads\/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06\.zip -text/);
  assert.match(readFileSync(publishedAttributesUrl, 'utf8'), /downloads\/MiniMax-H3-4Koma-Fused4Step-SLA-Bundle-2026-09-06\.zip\.sha256\.txt -text/);
  ...
  assert.equal(workflow.extra.distribution.variant, 'non-LLM Fused4step + SLA');
...
test('bundle preserves its corrected BGM contract, manifest, licensing, and no credential artifact', () => {
  ...
  assert.equal(advertisedHash, hashBytes(bundleBytes));
  const files = readZipFilesFromBuffer(bundleBytes);
  const root = 'Super-FURU-AI_四コマ_非LLM_Fused4step_SLA_配布用_20260906/';
  const expected = [
    'README_最初にお読みください.md', 'VERSION.txt', 'MANIFEST_SHA256.txt', 'LICENSE_ワークフローと独自ノード.txt', 'カスタムノード・ライセンスと出典.md', 'モデル一覧・取得先.md', 'セットアップ.ps1', '検証記録.json',
    '01_ワークフロー/【最新版・2026-09-06・非LLM・Fused4step＋SLA】四コマ_自動可変尺.json',
    '02_カスタムノード/ComfyUI-NanoBanana-H3/LICENSE', '02_カスタムノード/ComfyUI-NanoBanana-H3/README.md', '02_カスタムノード/ComfyUI-NanoBanana-H3/README_API_SECURITY.md', '02_カスタムノード/ComfyUI-NanoBanana-H3/__init__.py', '02_カスタムノード/ComfyUI-NanoBanana-H3/h3_prompt_system.txt', '02_カスタムノード/ComfyUI-NanoBanana-H3/identity_contract.py', '02_カスタムノード/ComfyUI-NanoBanana-H3/web/nanobanana_h3.js',
  ...
  assert.equal(hashBytes(files.get(`${root}01_ワークフロー/【最新版・2026-09-06・非LLM・Fused4step＋SLA】四コマ_自動可変尺.json`)), '1e26382f82eae197b45295d41317d70cff676b27ef1be1390098c60447b97576');
...
test('README matches the H3-BGM distribution and workflow does not describe credential persistence', () => {
  assert.match(readmeSource, /Fused4Step-SLA-2026-09-06/);
  ...
  assert.match(readmeSource, /H3 SLA Attention.*ComfyUI-PlagueKind-Nodes.*Triton/s);
