...
### Phase 0: 事前監査 (Pre-flight Intelligence Audit)
1. **APIモデルの死活チェック**: 現在利用している生成AIモデル（Gemini, OpenAI等）の提供終了日や非推奨化アナウンスがないかをWeb検索で確認する。
2. **仕様変更のキャッチアップ**: 画像生成などのAPI制約に変更がないか確認し、コード上の仕様（モデル名、パラメータ等）を最新状態にアップデートする。
3. **他プロジェクトの横断クロール**: モデル名の変更や廃止が見つかった場合、自律的に関連プロジェクト（`remotion_video`, `remotion_video_2` など）の環境設定やソースコードをチェックし、横断的な修正提案を行うこと。
4. **OpenAI APIの制限解除チェック**: OpenAIの画像APIにおいて「開発アプリ経由の画像生成（サードパーティ製アプリからのリクエスト）」が許可されるよう制限が解除されたかを確認する。もし解除されていれば、アプリ内からの直接画像生成機能を有効化する対応（UI表示の戻し等）を行う。
...
5. `node scripts/update_version.cjs` でバージョンインクリメント（※**パッチが `9` の次はマイナーを上げること。例: `4.0.9` の次は `4.1.0` とし、`4.0.10` は禁止**）
6. 3箇所の同期を**実ファイルで確認**: package.json, src/App.jsx, index.html
7. `README.md` の ChangeLog を更新する（変更内容を英日併記で追記、長すぎる場合は古いエントリを削除）
8. **HF側のREADME (`C:\Users\sx717\Antigravity\hf-nano-banana-pro\README.md`) を、新機能や変更点に応じてHFの流儀（一般ユーザー向け・YAML維持）でアップデートする。**
9. `git add . && git commit -m "Bump version and Deploy"`
...
### Phase 4: タグ・リリース
12. `git tag vX.Y.Z && git push origin --tags`  # ※vX.Y.Z は正式版。プレリリースの場合は vX.Y.Z-alpha 等
13. release_body.md にリリースノート作成（日英併記）
14. **タグ文章をユーザーに表示**:
    - タイトルをMarkdownコードブロックで表示（英語 / 日本語）
    - リリースノート本文の **日本語箇所** を必ずこのチャット画面上にも明確に表示すること。
15. `gh release create vX.Y.Z --title "..." -F release_body.md --latest`

> ⚠️ **`--prerelease` フラグは使用禁止。** リリースの際は必ず `--latest` を付けることで、GitHubサイドバーの「Latest」が正しく最新バージョンに更新されるようにすること。

### Phase 5: ZIP確認
16. `gh release view vX.Y.Z --json assets,zipballUrl` で最新版ZIPダウンロード可能か確認
...
- ❌ **【超重要】すべての変更（コミット・プッシュ）が完全に終わる前に、フルバックアップを実行・開始してはならない。** 今回のような「バックアップ開始直後に別の修正やドキュメント更新をコミットしてしまい、バックアップとGitの最新状態が食い違う（Stale Backup）」という致命的なミスを防ぐため、**「すべての開発・デプロイ・ドキュメント更新・コミット・プッシュ」が100%完了し、これ以上一切の変更がないことを確認してから**、最後の締めのプロセスとしてのみフルバックアップ（`backup_full.ps1`等）を起動すること。もしバックアップ中に新たな変更を加えた場合は、直ちに古いプロセスを停止（Kill）し、初めからバックアップを取り直すこと。
