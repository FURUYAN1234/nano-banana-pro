...
- **【必須】APIモデルの死活チェックと最新化（デプロイ前）**: Gemini APIやOpenAI APIのモデル（例: `gemini-1.5-pro` や `dall-e-3` 等）を扱う処理に触れる際、特に**デプロイを行う前の事前監査（Phase 0）として**、現在コードで指定されているモデルが非推奨化・アクセス不可になっていないか、より高性能な新規モデル（2.0や3.0など）が登場していないかをWeb検索で必ず確認すること。
...
- **【必須】他アプリ（横断プロジェクト）への影響クロール**: 本プロジェクト内でAPIモデルの変更が必要になった場合、同じ環境や関連プロジェクト（例: `remotion_video`, `remotion_video_2`, `comic-translation` 等）でも同様にモデルの非推奨化が起きていないかを自律的にクロール・横断チェックし、ユーザーに「他プロジェクトでも変更が必要か」を能動的に報告すること。
