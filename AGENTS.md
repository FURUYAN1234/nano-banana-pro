...
- **読込時UI**: ワークフロー読込完了時、プルダウンで選択中のProviderがそのComfyUIプロセスに未登録の場合だけ、そのProviderのAPI入力ダイアログを自動表示する。登録済みなら自動表示しない。
- **送信境界**: APIキーを送るのは、接続中のComfyUIサーバーから、利用者が選択したProviderの認証確認またはワークフロー実行を行う時だけとする。状態確認応答、ログ、例外、画面、ワークフロー出力へキー値を含めない。
- **配布前証拠**: 公開対象の全ワークフローJSON、外側ZIP、全ネストZIP、同梱コード、README、仕様、検証記録を実バイトで再帰確認する。旧資格情報ファイル名、ファイル読書き処理、平文キーらしき値、旧保存説明がないこと、および自動ダイアログとプロセスメモリ保持を回帰テストで証明する。ソースだけ、説明だけ、または外側ZIPだけの確認では合格にしない。
